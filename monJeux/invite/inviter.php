...
$cpt = (isset($_GET['id_pays']) ? intval($_GET['id_pays']) : 0);
...
$query = "SELECT * FROM mespays WHERE id_pays= $id_pays ";
$result = mysqli_query($connection, $query);
$rows = mysqli_fetch_assoc($result);
?>
<script>
if(<?php  echo json_encode($score);?> == 0 ) //remise à zero du score quand c'est une nouvelle partie
...
<html lang="fr">
	...
	<body>
		<div id="maDiv"  style="width: 1200px ; height: 450px;  display: block; margin-left: auto; margin-right: auto; position: absolute; transform: translate(-50%, -50%);top: 40%; left: 50%"></div>
		        <a href="../../profil.php" >
			...
			<h1 id="bravo"></h1>
			<h2 id="question_flag">Ce drapeau fait référence a quel pays ?</h2>
			...
			<div><input type="button" id="quitter" value="Quitter" onclick="myFunctionQ()": /></div>
			<div><input type="button" id="cnx" value="Pour plus de quiz connectez vous" onclick="myFunctionP()":  /></div>
		...
		<script>
			...
			function myFunction() { // fin de la partie (bouton rejouer)
				     javascript: document.location = '?id_pays=<?php echo ($cpt + 1);?>&sc='+0;
					 localStorage.setItem("score",0);
			...
			function myFunctionS() { //question suivante
				     javascript: document.location = '?id_pays=<?php echo ($cpt + 1);?>&sc='+localStorage.getItem("score");
			} 

			function myFunctionP() { // pour limiter les essaies 
				     javascript: document.location = '../../index.php#inscription';
					 localStorage.setItem("score",0);
			} 

		//	console.log(idd); 

			function limiterReponse() {
			...
			L.geoJson(countries, {style:function(feature) {return {color : "#000000" , fillColor : '#ffffff' , weight: 1}},
								  onEachFeature: function onEachFeature(feature, layer) {  // la on trace notre map + on affecte event handlers à chaque polygone
									layer.on('mouseover', function () { // un peu de style
      								this.setStyle({
    							    'fillColor': '#0000ff'
								      });});
									 ...
									 reponse  =	 feature.properties.cca2;
										...
										$.ajax({
                                                   ...
                                                   success: function(datas) {
													 ...
													 success: function(datas) {
													 if(	feature.properties.cca2 == datas.features[0].properties.cca2 ) // comparaison de la réponse
														 { // si réponse juste
															layer.on('mouseout', function () {
																this.setStyle({
																'fillColor': '#00FF00'
													 ...
													 else { $("#bravo").html("Désolé! Mauvaise réponse "); // si réponse fausse
													     	$("#bravo").css("color", "#D8000C");
														    $("#bravo").css("background-color", "#FFBABA");
														    $("#bravo").css("padding", "12px");
													        bonneReponse = 0;
															limiterReponse(); // passer au question suivante
														  }	 
													 },
