...
<html lang="fr">
	...
	<body>
		<div id="maDiv"  style="width: 1200px ; height: 450px;  display: block; margin-left: auto; margin-right: auto; position: absolute; transform: translate(-50%, -50%);top: 40%; left: 50%"></div>
			...
			<form method="post"><div class="p-2 "><span id="compteur"></span></div></form>
			</div>
			<h1 id="bravo"></h1>	
		    ...
		    <script>   
			 ...
			 var id_pays = sessionStorage.getItem("idPays");
				...
				<?php 	 if ($id_pays > 177 )  // fin de la partie
						 { 
		                    $id_pays=171; ?>
						    id_pays=0;
				            MonScore=0;
					...
					if($userexist == 1)
						   {
							  ...
							  if($userinfo['score_max'] < $score )
								  {   
									$query0 =$bdd->prepare("DELETE from parties where id = ? "); // mise à jour de la base
									$query0->execute(array($getid));
									$query2 =$bdd->prepare("INSERT INTO parties(id,score_max) VALUES(?, ?) ");
		...
		<script>
			...
			var map = new L.Map('maDiv', {
				center: [48.44081946034107, 17.608295669546813],
				minZoom: 2,
				maxZoom: 18,
				zoom: 3,  // mohamed a mis 5
				maxBounds: bornes,	
			});
